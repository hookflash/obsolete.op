...
TransmitMixer::~TransmitMixer()
{
    ...
    while (!_reverseAudioFrames.Empty())
    {
        AudioFrame* audioFrame = static_cast<AudioFrame*>(_reverseAudioFrames.First()->GetItem());
        _reverseAudioFrames.PopFront();
        delete audioFrame;
    }
    delete &_critSect;
                            ...
                            const WebRtc_UWord16 currentMicLevel)
{
    ...
    {
        RecordAudioToCallFile(_mixingFrequency);
    }
...
WebRtc_Word32
TransmitMixer::SetReverseAudioFrame(AudioFrame* frame)
{
    CriticalSectionScoped cs(&_critSect);
    ...
    WEBRTC_TRACE(kTraceInfo, kTraceVoice, VoEId(_instanceId, -1),
                 "TransmitMixer::SetReverseAudioFrame()");
  
    AudioFrame* newFrame = new AudioFrame();
    *newFrame = *frame;
    _reverseAudioFrames.PushBack(newFrame);
...
WebRtc_Word32 TransmitMixer::RecordAudioToCallFile(
    const WebRtc_UWord32 mixingFrequency)
{
    CriticalSectionScoped cs(&_critSect);
    if (_fileCallRecorderPtr == NULL)
    {
        WEBRTC_TRACE(kTraceWarning, kTraceVoice, VoEId(_instanceId, -1),
                     "TransmitMixer::RecordAudioToCallFile() filerecorder doesnot"
                     "exist");
        return -1;
    }
    
    while (_reverseAudioFrames.GetSize() > 10) {
        ...
        WEBRTC_TRACE(kTraceWarning, kTraceVoice, VoEId(_instanceId, -1),
                     "TransmitMixer::RecordAudioToCallFile() Reverse audio frame buffer overflow occured. "
                     "Old frame has been discarded.");
    }
    
    AudioFrame* reverseAudioFrame = NULL;
    
    if (!_reverseAudioFrames.Empty())
    {
        if (_audioFrame._payloadDataLengthInSamples != reverseAudioFrame->_payloadDataLengthInSamples)
        {
            reverseAudioFrame = static_cast<AudioFrame*>(_reverseAudioFrames.First()->GetItem());
        }
        else
        {
            WEBRTC_TRACE(kTraceWarning, kTraceVoice, VoEId(_instanceId, -1),
                         "TransmitMixer::RecordAudioToCallFile() reverse audio frame buffer length "
                         "is not equal to audio frame buffer length");
    ...
    {
        WEBRTC_TRACE(kTraceWarning, kTraceVoice, VoEId(_instanceId, -1),
                     "TransmitMixer::RecordAudioToCallFile() reverse audio frame buffer is empty");
    }

    if (reverseAudioFrame != NULL)
    {
        Utility::MixWithSat(reverseAudioFrame->_payloadData,
                            static_cast<int>(reverseAudioFrame->_audioChannel),
                            _audioFrame._payloadData,
                            static_cast<int>(_audioFrame._audioChannel),
                            static_cast<int>(_audioFrame._payloadDataLengthInSamples));
        
        if (_fileCallRecorderPtr->RecordAudioToFile(*reverseAudioFrame) != 0)
        {
            WEBRTC_TRACE(kTraceWarning, kTraceVoice, VoEId(_instanceId, -1),
                         "TransmitMixer::RecordAudioToCallFile() file recording"
                         "failed");
            delete reverseAudioFrame;
            return -1;
        }
