...
WebRtc_Word32
OutputMixer::SetTransmitMixer(
    TransmitMixer* transmitMixer)
{
    WEBRTC_TRACE(kTraceInfo, kTraceVoice, VoEId(_instanceId,-1),
                 "OutputMixer::SetTransmitMixer("
                 "transmitMixer=0x%x)", transmitMixer);
...
OutputMixer::DoOperationsOnCombinedSignal()
{
    ...
    // --- Set Audio Frame for call recording
    TMSetReverseAudioFrame();
...
int
OutputMixer::TMSetReverseAudioFrame()
{
    int outLen(0);
    AudioFrame audioFrame = _audioFrame;
    ...
    int mixingFrequency = _transmitMixerPtr->GetMixingFrequency();
  
    if (mixingFrequency == 0)
        return 0; 
  
    if (audioFrame._audioChannel == 1)
    {
        _tmResampler.ResetIfNeeded(_audioFrame._frequencyInHz,
                                    mixingFrequency,
                                    kResamplerSynchronous);
    }
    else
        ...
        outLen) == 0)
    {
        audioFrame._payloadDataLengthInSamples =
        (outLen / _audioFrame._audioChannel);
        audioFrame._frequencyInHz = mixingFrequency;
    }
    ...
    if (_transmitMixerPtr->SetReverseAudioFrame(&audioFrame) == -1)
    {
        WEBRTC_TRACE(kTraceWarning, kTraceVoice, VoEId(_instanceId,-1),
                     "TransmitMixer::SetReverseAudioFrame() => error");
    }
