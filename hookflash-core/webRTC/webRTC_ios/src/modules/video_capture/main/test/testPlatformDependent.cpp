...

{
    ...
    _captureInfo = videocapturemodule::VideoCaptureImpl::CreateDeviceInfo(5);
...
WebRtc_Word32 testPlatformDependent::testCreateDelete(
    const char* uniqueID)
{
    WebRtc_Word32 testTime = 60000;//8000;
    ...
    for (WebRtc_Word32 i = 0; i < numOfLoops; ++i)
    {
...
#else
        _captureSettings[0].capability.width = 640;
        _captureSettings[0].capability.height = 480;
        ...
        _captureSettings[0].captureModule = videocapturemodule::VideoCaptureImpl::Create(0,
                                                                       uniqueID);
        ...
        while (_captureSettings[0].incomingFrames <= 50
...
WebRtc_Word32 testPlatformDependent::testMultipleCameras()
{
    ...
    for (WebRtc_UWord32 i = 0; i < _noOfCameras; ++i)
    {
        ...
        if (!_renderer[i])
        {
            _renderer[i] = new Renderer(NULL, true);
        }
#endif
        char id[256];
        _captureInfo->GetDeviceName(i, _captureSettings[i].captureName, 256,
                                    id, 256);
        char* name = _captureSettings[i].captureName;
...
WebRtc_Word32 testPlatformDependent::testRotation(const char* uniqueID)
{
...
int testPlatformDependent::DoTest(IPhoneRenderView* renderView)
{
    LOG("\ntestPlatformDependent::DoTest()\n");
#ifdef RENDER_PREVIEW
    if (!_renderer[0])
    {
        _renderer[0] = new Renderer(renderView, true);
    }
#endif
    ...
    for (WebRtc_UWord32 i = 1; i < _noOfCameras; ++i)
    {
        char name[256];
        char uniqueID[256];
        char productId[256];
