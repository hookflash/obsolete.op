...
public:
    ...
    int DoTest(IPhoneRenderView* renderView);
...
private:
    ...
    WebRtc_Word32 testCreateDelete(const char* uniqueID);
    WebRtc_Word32 testCapabilities(const char* uniqueID);
    WebRtc_Word32 testMultipleCameras();
    WebRtc_Word32 testRotation(const char* uniqueID);
