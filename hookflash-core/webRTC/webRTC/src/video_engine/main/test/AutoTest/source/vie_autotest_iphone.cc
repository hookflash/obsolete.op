...
int hookflash_test(const char* directory, int testID, 
                   const char* sndIpAddress, int sndPort, int rcvPort,
                   IPhoneRenderView* view1, IPhoneRenderView* view2, int width, int height)
{
...
bool TestThread(ThreadObj obj)
{
    ...
    testErrors += vieAutoTest->ViECodecStandardTest(captureWidth, captureHeight, 
