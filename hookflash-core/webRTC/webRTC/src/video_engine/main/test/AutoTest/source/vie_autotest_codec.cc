...
int ViEAutoTest::ViECodecStandardTest(int width, int height, 
                                      const char* sendIpAddress, int sendPort, int receivePort)
{
    ...
    tbCaptureDevice captureDevice = tbCaptureDevice(interfaces, numberOfErrors, width, height);
    int captureId = captureDevice.captureId;
    _vieCaptureId = captureId;
    ...
    ptrViE->SetTraceCallback(this);
  
    _vieBase = ptrViEBase;
    _vieCapture = ptrViECapture;
    _vieCodec = ptrViECodec;
    _vieNetwork = ptrViENetwork;
    _vieRtpRtcp = ptrViERtpRtcp;
    ...
    _vieChannel = videoChannel;
  
    ptrViEBase->RegisterObserver(*this);
    ptrViECapture->RegisterObserver(captureId, *this);
    ptrViECodec->RegisterEncoderObserver(videoChannel, *this);
    ptrViECodec->RegisterDecoderObserver(videoChannel, *this);
    ptrViENetwork->RegisterObserver(videoChannel, *this);
    ptrViERtpRtcp->RegisterRTPObserver(videoChannel, *this);
    ptrViERtpRtcp->RegisterRTCPObserver(videoChannel, *this);
    ...
    error = ptrViENetwork->SetMTU(videoChannel, 576);
...
//    const char* ipAddress = sendIpAddress;
    const char* ipAddress = "127.0.0.1";
//    const unsigned short sendRtpPort = sendPort;
    const unsigned short sendRtpPort = 6000;
//    const unsigned short receiveRtpPort = receivePort;
    const unsigned short receiveRtpPort = 6000;
    error = ptrViENetwork->SetLocalReceiver(videoChannel, receiveRtpPort);
    numberOfErrors += ViETest::TestError(error == 0, "ERROR: %s at line %d",
                                         __FUNCTION__, __LINE__);
    ...
    error = ptrViENetwork->SetSendDestination(videoChannel, ipAddress, sendRtpPort);
    numberOfErrors += ViETest::TestError(error == 0, "ERROR: %s at line %d",
                                         __FUNCTION__, __LINE__);
...
int ViEAutoTest::ViECodecExtendedTest()
{
    ...
    //
    {
        ...
        tbCaptureDevice tbCapture(ViE, numberOfErrors, 0, 0);
...
void ViEAutoTest::BrightnessAlarm(const int captureId, const Brightness brightness)
{
    if (_eventCallback != NULL)
...
void ViEAutoTest::NoPictureAlarm(const int captureId, const CaptureAlarm alarm)
{
    if (_eventCallback != NULL)
...
void ViEAutoTest::IncomingCodecChanged(const int videoChannel, const VideoCodec& videoCodec)
{
    if (_eventCallback != NULL)
...
void ViEAutoTest::PacketTimeout(const int videoChannel, const ViEPacketTimeout timeout)
{
    if (_eventCallback != NULL)
...
void ViEAutoTest::Print(const TraceLevel level, const char *traceString, const int length)
{
    printf("WEBRTC LOG: %s\n", traceString);
}
