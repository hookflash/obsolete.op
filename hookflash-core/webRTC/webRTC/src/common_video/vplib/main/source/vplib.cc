...
#ifdef MAC_IPHONE
WebRtc_Word32 
ConvertI420ToABGR_LIBYUV(const WebRtc_UWord8* inFrame, WebRtc_UWord8* outFrame,
                         WebRtc_UWord32 width, WebRtc_Word32 height,
                         WebRtc_UWord32 strideOut)
{
    if (width < 1)
    {
    ...
    WebRtc_UWord32 src_stride_y, src_stride_u, src_stride_v, dst_stride_argb;
    const WebRtc_UWord8 *src_y, *src_u, *src_v;
    WebRtc_UWord8 *dst_argb;
    ...
    void (*FastConvertYUVToABGRRow)(const WebRtc_UWord8* y_buf,
                                    const WebRtc_UWord8* u_buf,
                                    const WebRtc_UWord8* v_buf,
                                    WebRtc_UWord8* rgb_buf,
                                    WebRtc_UWord32 width);
  
    FastConvertYUVToABGRRow = FastConvertYUVToARGBRow_NEON;

    WebRtc_UWord32 residue = width % 16;
    ...
    for (int y = 0; y < height; ++y) 
    {
        ...
        {
            dst_argb[4*(width-i)+0] = 0x00;
            dst_argb[4*(width-i)+1] = 0x00;
            dst_argb[4*(width-i)+2] = 0x00;
...
WebRtc_Word32 
ConvertABGRToI420_LIBYUV(WebRtc_UWord32 width, WebRtc_Word32 height,
                         const WebRtc_UWord8* inFrame, WebRtc_UWord8* outFrame)
{
    ...
    WebRtc_UWord32 src_stride_argb, dst_stride_y, dst_stride_u, dst_stride_v;
    const WebRtc_UWord8 *src_argb;
    WebRtc_UWord8 *dst_y, *dst_u, *dst_v;
    ...
    void (*FastConvertABGRToYUVRow2)(const WebRtc_UWord8* rgb_buf,
                                     WebRtc_UWord8* y_buf,
                                     WebRtc_UWord8* u_buf,
                                     WebRtc_UWord8* v_buf,
                                     WebRtc_UWord32 width,
    ...
    FastConvertABGRToYUVRow2 = FastConvertABGRToYUVRow2_NEON;
    ...
    for (int y = 0; y < height/2; ++y) 
    {
        ...
        {
            const WebRtc_UWord8* r1_px = src_argb + (width-i)*4 + 0;
            const WebRtc_UWord8* g1_px = src_argb + (width-i)*4 + 1;
            const WebRtc_UWord8* b1_px = src_argb + (width-i)*4 + 2;
            const WebRtc_UWord8* r2_px = src_argb + (2*width-i)*4 + 0;
            const WebRtc_UWord8* g2_px = src_argb + (2*width-i)*4 + 1;
            const WebRtc_UWord8* b2_px = src_argb + (2*width-i)*4 + 2;
            WebRtc_UWord16 y1_px = ((77 * r1_px[0] + 150 * g1_px[0] + 29 * b1_px[0] - 32768) >> 8) + 128;
            WebRtc_UWord16 y2_px = ((77 * r1_px[4] + 150 * g1_px[4] + 29 * b1_px[4] - 32768) >> 8) + 128;
            WebRtc_UWord16 y3_px = ((77 * r2_px[0] + 150 * g2_px[0] + 29 * b2_px[0] - 32768) >> 8) + 128;
            WebRtc_UWord16 y4_px = ((77 * r2_px[4] + 150 * g2_px[4] + 29 * b2_px[4] - 32768) >> 8) + 128;
            dst_y[width-i+0] = Clip(y1_px);
            dst_y[width-i*4+1] = Clip(y2_px);
            dst_y[2*width-i*4+0] = Clip(y3_px);
            dst_y[2*width-i*4+1] = Clip(y4_px);
            WebRtc_UWord8 rm_px = (r1_px[0] + r1_px[4] + r2_px[0] + r2_px[4]) >> 2;
            WebRtc_UWord8 gm_px = (g1_px[0] + g1_px[4] + g2_px[0] + g2_px[4]) >> 2;
            WebRtc_UWord8 bm_px = (b1_px[0] + b1_px[4] + b2_px[0] + b2_px[4]) >> 2;
            WebRtc_UWord16 u_px = ((-43 * rm_px - 85 * gm_px + 128 * bm_px) >> 8) + 128;
            WebRtc_UWord16 v_px = ((128 * rm_px - 107 * gm_px - 128 * bm_px) >> 8) + 128;
...
WebRtc_Word32
ScaleRGBAFrameDouble_LIBYUV(WebRtc_UWord32 width, WebRtc_UWord32 height,
                            WebRtc_UWord8* inFrame)
{
    ...
    {
        // 2 rows
        ScaleDownRGBARow2_NEON(inPtr, width, outPtr, width >> 1);
        inPtr += width << 3;
        outPtr += width << 1; 
    }
...
WebRtc_Word32
ConvertNV12ToI420AndScaleFrameDouble_LIBYUV(WebRtc_UWord32 width, WebRtc_UWord32 height,
                                             WebRtc_UWord8* inFrame)
{
    if (width < 1 || height < 1)
    {
    ...
    {
        // 2 rows
        ScaleDownRow2_NEON(inPtr, width, outPtr, width >> 1);
        inPtr += width << 1;
        outPtr += width >> 1; 
    }
    ...
    {
        // 2 rows
        ScaleDownAndDeinterleaveRow2_NEON(inPtr, width >> 1, outPtr, outPtr + (width * height >> 4), width >> 2);
        inPtr += width << 1;
        outPtr += width >> 2; 
    }
...
typedef void (*rotate_wx8_func)(const WebRtc_UWord8*, WebRtc_Word32, WebRtc_UWord8*, WebRtc_Word32, WebRtc_Word32);
typedef void (*rotate_wxh_func)(const WebRtc_UWord8*, WebRtc_Word32, WebRtc_UWord8*, WebRtc_Word32, WebRtc_Word32, WebRtc_Word32);

static void TransposeWxH_C(const WebRtc_UWord8* src, WebRtc_Word32 src_stride,
...
void TransposePlane(const WebRtc_UWord8* src, WebRtc_Word32 src_stride,
                    WebRtc_UWord8* dst, WebRtc_Word32 dst_stride,
                    WebRtc_Word32 width, WebRtc_Word32 height) {
    int i = height;
...
WebRtc_Word32 RotatePlane90_LIBYUV(const WebRtc_UWord8* src, WebRtc_Word32 src_stride,
                          WebRtc_UWord8* dst, WebRtc_Word32 dst_stride,
                          WebRtc_Word32 width, WebRtc_Word32 height) {
...
WebRtc_Word32 RotatePlane270_LIBYUV(const WebRtc_UWord8* src, WebRtc_Word32 src_stride,
                           WebRtc_UWord8* dst, WebRtc_Word32 dst_stride,
                           WebRtc_Word32 width, WebRtc_Word32 height) {
...
WebRtc_Word32 RotatePlane180_LIBYUV(const WebRtc_UWord8* src, WebRtc_Word32 src_stride,
                           WebRtc_UWord8* dst, WebRtc_Word32 dst_stride,
                           WebRtc_Word32 width, WebRtc_Word32 height) {
    void (*ReverseRow)(const WebRtc_UWord8* src, WebRtc_UWord8* dst, WebRtc_Word32 width);
...
WebRtc_Word32
ConvertABGRMacToI420(WebRtc_UWord32 width, WebRtc_UWord32 height,
                     const WebRtc_UWord8* inFrame, WebRtc_UWord8* outFrame)
{
    if (height < 1 || width < 1)
    {
...
*/
            ...
            yStartPtr2[0] = (WebRtc_UWord8)((66 * inpPtr2[2] + 129 * inpPtr2[1]
                                             + 25 * inpPtr2[0] + 128) >> 8) + 16;
            // moving to next column
            yStartPtr[1] =  (WebRtc_UWord8)((66 * inpPtr[6] + 129 * inpPtr[5]
                                             + 25 * inpPtr[4] + 128) >> 8) + 16;
            yStartPtr2[1] = (WebRtc_UWord8)((66 * inpPtr2[6] + 129 * inpPtr2[5]
                                             + 25 * inpPtr2[4] + 128) >> 8) + 16;
            //U
            uStartPtr[0] = (WebRtc_UWord8)((-38 * inpPtr[2] - 74 * inpPtr[1]
                                            + 112 * inpPtr[0] + 128) >> 8) + 128;
            //uStartPtr[0] = 0;
...
#ifdef MAC_IPHONE
WebRtc_Word32 RotateI420(const WebRtc_UWord8* inBuffer, WebRtc_UWord8* outBuffer, WebRtc_Word32 width, WebRtc_Word32 height,
                         VideoRotationMode rotateFrame)
{
    if (rotateFrame == kRotateClockwise)
    {
        // Y plane
        WebRtc_Word32 rotationResult = RotatePlane90_LIBYUV(inBuffer, width,
                                                            outBuffer, height,
                                                            width, height);
        if (rotationResult < 0)
            return -1;
        // U plane
        rotationResult = RotatePlane90_LIBYUV(inBuffer + width * height, width / 2,
                                              outBuffer + width * height, height / 2,
                                              width / 2, height / 2);
        if (rotationResult < 0)
            return -1;
        // V plane
        rotationResult = RotatePlane90_LIBYUV(inBuffer + width * height + width * height / 4, width / 2,
                                              outBuffer + width * height + width * height / 4, height / 2,
                                              width / 2, height / 2);
        if (rotationResult < 0)
            return -1;
    }
    else if (rotateFrame == kRotate180)
    {
        // Y plane
        WebRtc_Word32 rotationResult = RotatePlane180_LIBYUV(inBuffer, width,
                                                             outBuffer, width,
                                                             width, height);
        if (rotationResult < 0)
            return -1;
        // U plane
        rotationResult = RotatePlane180_LIBYUV(inBuffer + width * height, width / 2,
                                               outBuffer + width * height, width / 2,
                                               width / 2, height / 2);
        if (rotationResult < 0)
            return -1;
        // V plane
        rotationResult = RotatePlane180_LIBYUV(inBuffer + width * height + width * height / 4, width / 2,
                                               outBuffer + width * height + width * height / 4, width / 2,
                                               width / 2, height / 2);
        if (rotationResult < 0)
            return -1;
    }
    else if (rotateFrame == kRotateAntiClockwise)
    {
        // Y plane
        WebRtc_Word32 rotationResult = RotatePlane270_LIBYUV(inBuffer, width,
                                                             outBuffer, height,
                                                             width, height);
        if (rotationResult < 0)
            return -1;
        // U plane
        rotationResult = RotatePlane270_LIBYUV(inBuffer + width * height, width / 2,
                                               outBuffer + width * height, height / 2,
                                               width / 2, height / 2);
        if (rotationResult < 0)
            return -1;
        // V plane
        rotationResult = RotatePlane270_LIBYUV(inBuffer + width * height + width * height / 4, width / 2,
                                               outBuffer + width * height + width * height / 4, height / 2,
                                               width / 2, height / 2);
        if (rotationResult < 0)
            return -1;
    ...
    {
        memcpy(outBuffer, inBuffer, width * height * 3 / 2);
    }
...
#ifdef MAC_IPHONE
        case kARGB:
//            outgoingLength = ConvertABGRMacToI420(width, height, incomingBuffer,
//                                                  outgoingBuffer);
            {
                WebRtc_UWord8* buffer = new WebRtc_UWord8[width*height*3/2];
                outgoingLength = ConvertABGRToI420_LIBYUV(width, height, incomingBuffer,
                                                          buffer);
                outgoingLength = RotateI420(buffer, outgoingBuffer, width, height, rotate);
                delete [] buffer;
            }
            break;
#endif // MAC_IPHONE
        case kI420:
#ifdef MAC_IPHONE
            outgoingLength = RotateI420(incomingBuffer, outgoingBuffer, width, height, rotate);
#else
            switch(rotate)
                {
                    case kRotateNone:
