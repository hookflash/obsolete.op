...
int VoEExtendedTest::TestCodec(const char* sndIpAddress, int sndPort, int rcvPort)
{
    ...
    VoEFile* file = _mgr.FilePtr();
    VoEAudioProcessing* audioProcessing = _mgr.APMPtr();
...
//                                              kTraceMemory |
//                                              kTraceInfo));
    TEST_MUSTPASS(VoiceEngine::SetTraceFilter(kTraceAll));
#endif

    TEST_MUSTPASS(base->Init());
    TEST_MUSTPASS(base->CreateChannel() != 0);
//    TEST_MUSTPASS(base->CreateChannel() != 1);
...
#else
    TEST_MUSTPASS(base->SetSendDestination(0, 12345, "127.0.0.1", 20000));
    TEST_MUSTPASS(base->SetLocalReceiver(0, 12345));
    //TEST_MUSTPASS(base->SetSendDestination(0, sndPort, sndIpAddress));
    //TEST_MUSTPASS(base->SetLocalReceiver(0, rcvPort));
#endif

    ///////////////////////////
    // Actual test starts here

    //audioProcessing->SetAgcStatus(true, kAgcAdaptiveDigital);
    //audioProcessing->SetEcStatus(true, kEcAecm);
    //audioProcessing->SetAecmMode(kAecmSpeakerphone);
