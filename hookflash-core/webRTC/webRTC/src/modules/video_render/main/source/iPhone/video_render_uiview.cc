...
int VideoChannelUIView::DeliverFrame(unsigned char* buffer, int bufferSize, unsigned int /*timeStamp90kHz*/)
{
    ...
    ConvertI420ToABGR_LIBYUV(buffer, captureFrame.Buffer(), _width, _height, 0);
    ...
    if (_owner->getWindowReference().viewId == 0)
        [_owner->getWindowReference()->delegate SendLastFrameToMainView:image :_owner->getWindowReference()];
    else
        [_owner->getWindowReference()->delegate SendLastFrameToSubView:image :_owner->getWindowReference()];
...
VideoRenderUIView::VideoRenderUIView(IPhoneRenderView *windowRef, bool fullScreen, int iId) :
_windowRef( (IPhoneRenderView*)windowRef),
...
int VideoRenderUIView::ChangeWindow(IPhoneRenderView* newWindowRef)
{
...
bool VideoRenderUIView::IsFullScreen()
{
