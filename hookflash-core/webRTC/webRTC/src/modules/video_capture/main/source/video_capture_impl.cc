                                                    ...
                                                    WebRtc_Word64 captureTime/*=0*/)
{
    ...
    if (frameInfo.codecType == kVideoCodecUnknown) // None encoded. Convert to I420.
    {
        ...
        {
            WEBRTC_TRACE(webrtc::kTraceError, webrtc::kTraceVideoCapture, _id,
                       "Failed to allocate I420 frame buffer.");
            return -1;
        }
#elif defined RGBA_CAPTURE
        // Allocate ARGB buffer
        _captureFrame.VerifyAndAllocate(CalcBufferSize(kARGB, width, height));
        if (!_captureFrame.Buffer())
        {
            WEBRTC_TRACE(webrtc::kTraceError, webrtc::kTraceVideoCapture, _id,
                         "Failed to allocate ARGB frame buffer.");
            return -1;
        }
#endif
        ...
        if (_rotateFrame == kRotateNone || _rotateFrame == kRotate180)
        {
        ...
        {
            _captureFrame.SetWidth(height);
            _captureFrame.SetHeight(width);
        }
