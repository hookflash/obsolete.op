...
int WebRtcCreateWindow(UIViewRenderer*& uiViewRenderer,int winNum, int width, int height)
{
...
/*
    ...
    [outWindow setTitle:@"UIView Renderer"];
...
*/
    ...
    //NSRect uiViewRendererFrame = NSMakeRect(10, 10, width, height);
    uiViewRenderer = [[UIImageView alloc] initWithFrame:[window bounds]];
    [window addSubview:uiViewRenderer];

    // must tell UIView to draw the window, but any GUI work must be done on the main thread.
...
//    [outWindow makeKeyAndOrderFront:NSApp];
    ...
    [uiViewRenderer performSelectorOnMainThread:@selector(setImage:) withObject:nil waitUntilDone:YES];
...
void SetWindowPos(UIViewRenderer*& uiViewRenderer, int x, int y, int width, int height, bool onTop)
{
    //NSWindow* ownerWindow = [uiViewRenderer window];
...
Renderer::Renderer(IPhoneRenderView* renderView, bool preview) :
    _renderModule(NULL), _quiting(false), _renderView(renderView)
...
*/
    ...
    _renderModule = VideoRender::CreateVideoRender(0, (void*) _renderView,
                                                   false);
...
bool Renderer::RenderThreadProcess()
{
    ...
    if (_quiting == false && _renderView == NULL) // Create the render window
    {
