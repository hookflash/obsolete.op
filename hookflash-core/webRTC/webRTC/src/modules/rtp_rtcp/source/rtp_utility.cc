...
bool
ModuleRTPUtility::RTPPayloadParser::Parse( RTPPayload& parsedPacket, bool markerBit) const
{
    ...
    case kRtpVp8Video:
        //return ParseVP8(parsedPacket, markerBit);
        return ParseVP8_old(parsedPacket);
    default:
        return false;
    }
}
...
bool ModuleRTPUtility::RTPPayloadParser::ParseVP8(RTPPayload& parsedPacket, bool markerBit) const {
    ...
    vp8->beginningOfFrame = vp8->beginningOfPartition;
    if (!vp8->beginningOfPartition || !markerBit)
    {
        parsedPacket.info.VP8.fragments = true;
        if (vp8->beginningOfPartition)
        {
    ...
    {
        parsedPacket.info.VP8.fragments = false;
        parsedPacket.info.VP8.startFragment = true;
        parsedPacket.info.VP8.stopFragment = true;
    }
...
int ModuleRTPUtility::RTPPayloadParser::ParseVP8FrameSize(RTPPayload& parsedPacket,
...
int ModuleRTPUtility::RTPPayloadParser::ParseVP8Extension(RTPPayloadVP8* vp8,
                                                          const WebRtc_UWord8* dataPtr,
                                                          int dataLength) const {
    int parsedBytes = 0;
...
int ModuleRTPUtility::RTPPayloadParser::ParseVP8PictureID(RTPPayloadVP8* vp8,
                                                          const WebRtc_UWord8** dataPtr,
                                                          int* dataLength,
...
int ModuleRTPUtility::RTPPayloadParser::ParseVP8Tl0PicIdx(RTPPayloadVP8* vp8,
                                                          const WebRtc_UWord8** dataPtr,
                                                          int* dataLength,
                                                          int* parsedBytes) const {
    if (*dataLength <= 0) return -1;
...
int ModuleRTPUtility::RTPPayloadParser::ParseVP8TIDAndKeyIdx(RTPPayloadVP8* vp8,
                                                             const WebRtc_UWord8** dataPtr,
                                                             int* dataLength,
                                                             int* parsedBytes) const {
    if (*dataLength <= 0) return -1;
...
bool
ModuleRTPUtility::RTPPayloadParser::ParseVP8_old(RTPPayload& parsedPacket) const
{
    ...
    if(parsedPacket.info.VP8.hasPictureID)
    {
        ...
        if (_dataPtr[numBytesPictureId] & 0x80)
        {
            numBytesPictureId++;
        }
