...
namespace zsLib
{
  ...
  namespace XML
  {
    ...
    namespace internal
    {
      ...
      void Declaration::parse(XML::ParserPos &ioPos)
      {
               ...
               (!ioPos.isString("?>")))
        {
              ...
              (ioPos.isString("/>")))
          {
            ...
            (ioPos.getDocument())->addWarning(ParserWarningType_NotProperEndDeclaration, ioPos);
            if (ioPos.isString("/>"))
              ioPos += (ULONG)strlen("/>");
            else
              ++ioPos;
        ...
        if (!ioPos.isString("?>"))
        {
          (ioPos.getDocument())->addWarning(ParserWarningType_NoEndDeclarationFound);
          return;
        }
      ...
      ULONG Declaration::getOutputSize(const DocumentPtr &inDocument) const
      {
        ULONG result = 0;
        ...
        if (mFirstAttribute)
        {
          ...
          while (child)
          {
            ...
            result += (ULONG)strlen(" ");
            result += Parser::getOutputSize(inDocument, attribute);
      ...
      void Declaration::writeBuffer(const DocumentPtr &inDocument, char * &ioPos) const
      {
        Parser::writeBuffer(ioPos, "<?xml");
        ...
        if (mFirstAttribute)
        {
          ...
          while (child)
          {
            ...
            Parser::writeBuffer(inDocument, attribute, ioPos);
            child = child->getNextSibling();
          }
        }
        Parser::writeBuffer(ioPos, " ?>");
