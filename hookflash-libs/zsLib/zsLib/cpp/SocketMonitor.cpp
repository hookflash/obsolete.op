...
#include <zsLib/internal/SocketMonitor.h>
...
namespace zsLib
{
  namespace internal
  {
    ...
    class SocketMonitorGlobalInit;

    class SocketMonitorGlobal
    {
    public:
      SocketMonitorGlobal(SocketMonitorGlobalInit &) : mSocketMonitor(new SocketMonitor) { }
      ~SocketMonitorGlobal() { mSocketMonitor->waitForShutdown(); }

      SocketMonitorPtr &get() {return mSocketMonitor;}

    private:
      SocketMonitorPtr mSocketMonitor;
    };

    static SocketMonitorPtr getSocketMonitor();

    class SocketMonitorGlobalInit
    {
    public:
      SocketMonitorGlobalInit() {getSocketMonitor();}
      ~SocketMonitorGlobalInit() {}
    };

    static SocketMonitorGlobalInit gSocketMonitorGlobalInit;        // this symbol isn't used but forces the object to be created

    static SocketMonitorPtr getSocketMonitor()
    {
      static SocketMonitorGlobal global(gSocketMonitorGlobalInit);  // forces a reference to the static global variable
      return global.get();
    }
                                     ...
                                     )
    {
      EventPtr event;
      {
        ...
        if (!mThread) {
          mThread = ThreadPtr(new boost::thread(boost::ref(*getSocketMonitor().get())));
#ifndef _WIN32
    ...
    void SocketMonitor::waitForShutdown()
    {
      ThreadPtr thread;
      {
        AutoRecursiveLock lock(mLock);
