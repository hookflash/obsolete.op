...
namespace zsLib
{
  ...
  namespace XML
  {
    ...
    ParserPos::ParserPos(Document &inDocument) :
    mRow(0),
    mColumn(0),
    mPos(NULL)
    {
      setDocument(inDocument.mThis.lock());
      setSOF();
    }

    ParserPos::ParserPos(DocumentPtr inDocument) :
    mRow(0),
    mColumn(0),
    mPos(NULL)
    {
    ...
    bool ParserPos::isSOF() const
    {
      ...
      ZS_THROW_BAD_STATE_IF(!document)

      return (mPos == document->mSOF);
    }
      ...
      while (0 != inDistance)
      {
        ...
        switch (*mPos)
        {
          ...
          case '\t':
          {
            ...
            ULONG tabSize = document->getTabSize();
      ...
      while (0 != inDistance)
      {
        ...
        switch (*mPos)
        {
          ...
          case '\t':
          {
            ...
            ZS_THROW_BAD_STATE_IF(!document)
            ZS_THROW_BAD_STATE_IF(NULL == document->mSOF)
            ...
            mColumn = internal::calculateColumnFromSOL(mPos, document->mSOF, document->mTabSize);
            break;
          }
          default:
          {
