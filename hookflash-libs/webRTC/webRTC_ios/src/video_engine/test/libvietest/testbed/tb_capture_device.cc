...
TbCaptureDevice::TbCaptureDevice(TbInterfaces& Engine, int captureIndex) :
    captureId(-1),
    ViE(Engine),
    vcpm_(NULL)
{
    ...
    size_t captureIdx = (size_t)captureIndex;
    ...
    else 
    {
        ...
        vcpm_ = webrtc::VideoCaptureFactory::Create(captureIdx, uniqueId);
        vcpm_->AddRef();
        int error = ViE.capture->AllocateCaptureDevice(*vcpm_, captureId);
        if (error == 0)
            captureDeviceSet = true;
