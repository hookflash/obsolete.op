...
WebRtc_Word32 AudioDeviceIPhone::Terminate() {
    ...
    OSStatus result = AudioSessionRemovePropertyListenerWithUserData(kAudioSessionProperty_AudioRouteChange,
                                                                     PropertyListenerCallback, this);
    if (0 != result) {
        WEBRTC_TRACE(kTraceInfo, kTraceAudioDevice, _id,
                     "Could not remove property listener (result=%d)", result);
    }
