...
void VideoRenderOpenGles20::UpdateTextures(const I420VideoFrame& frameToRender) {

  const GLsizei width = frameToRender.width();
  const GLsizei height = frameToRender.height();

  int bufferSize = CalcBufferSize(kI420, width, height);
  ...
  ExtractBuffer(frameToRender, captureFrame.Length(), captureFrame.Buffer());

  if (_textureWidth != width || _textureHeight != height) {
    SetupTextures(captureFrame);
  }
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, width, height, 0,
               GL_LUMINANCE, GL_UNSIGNED_BYTE,
               (const GLvoid*) captureFrame.Buffer());
  glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, width, height, GL_LUMINANCE,
                  GL_UNSIGNED_BYTE, (const GLvoid*) captureFrame.Buffer());

  currentTextureId = _textureIds[1]; // U
  glActiveTexture( GL_TEXTURE1);
  glBindTexture(GL_TEXTURE_2D, currentTextureId);
  const WebRtc_UWord8* uComponent = captureFrame.Buffer() + width * height;
  glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, width / 2, height / 2, 0,
               GL_LUMINANCE, GL_UNSIGNED_BYTE, (const GLvoid*) uComponent);
