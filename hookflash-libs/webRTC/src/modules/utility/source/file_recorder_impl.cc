...
#ifdef WEBRTC_MODULE_UTILITY_VIDEO
        return new MP4Recorder(instanceID, fileFormat);
#else
        WEBRTC_TRACE(kTraceError, kTraceFile, -1,
...
WebRtc_Word32 FileRecorderImpl::StopRecording()
{
    ...
    if (STR_CASE_CMP(codec_info_.plname, "vorbis") == 0)
    {
        if (_audioEncoder.EncodeVorbisEndOfStream(_audioBuffer, encodedLenInBytes) == -1)
        {
            WEBRTC_TRACE(
                         kTraceWarning,
                         kTraceVoice,
                         _instanceID,
                         "FileRecorder::StopRecording() codec %s not supported or\
    ...
    const TickTime* playoutTS)
{
 ...
 on");
    ...
    if (_fileFormat == kFileFormatPreencodedFile ||
        (STR_CASE_CMP(codec_info_.plname, "L16") != 0 && _fileFormat != kFileFormatMP4File))
    {
...
WebRtc_Word32 MP4Recorder::RecordVideoToFile(const VideoFrame& videoFrame)
{
    CriticalSectionScoped lock(_critSec);
...
WebRtc_Word32 MP4Recorder::EncodeAndWriteVideoToFile(VideoFrame& videoFrame)
{
    if(!IsRecording() || (videoFrame.Length() == 0))
    {
        return -1;
