...
WebRtc_Word32 AudioCodingModuleImpl::EncodeVorbisEndOfStream(WebRtc_Word8* encodedData,
                                                             WebRtc_UWord32& encodedLengthInBytes) {
  CriticalSectionScoped lock(*_acmCritSect);
    
  if (!HaveValidEncoder("EncodeVorbisEndOfStream")) {
    return -1;
  }

  return _codecs[_currentSendCodecIdx]->EncodeVorbisEndOfStream(encodedData, encodedLengthInBytes);
}
